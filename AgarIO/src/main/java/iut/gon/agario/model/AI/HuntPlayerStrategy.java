...
public class HuntPlayerStrategy implements AIDecisionStrategy {
    public static HashMap<String, Double> eat(AIPlayer aiPlayer, Player player, GameWorld gameWorld) {
        ...
        for(CompositePlayer cell : aiPlayer.getCompositePlayer()){
            for(CompositePlayer playerCell : player.getCompositePlayer()) {
                if (gameWorld.canAbsorb(playerCell, cell.getPlayers().get(0))) {
